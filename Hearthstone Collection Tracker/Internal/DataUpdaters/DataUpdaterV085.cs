...
{
    class DataUpdaterV085 : IDataUpdater
    {
        private static readonly Version _version = new Version(0, 8, 5);
        ...
        {
            const string RumbleSet = SetNames.RastakhansRumble;
            ...
            foreach (var file in Directory.GetFiles(HearthstoneCollectionTrackerPlugin.PluginDataDir, "Collection_*.xml", SearchOption.TopDirectoryOnly))
            {
                ...
                if (setInfos.Any(s => s.SetName == RumbleSet))
                    continue;
                ...
                {
                    SetName = RumbleSet,
                    Cards = cards.Where(c => c.CardSet == CardSet.TROLL).Select(c => new CardInCollection()
                    {
                        AmountGolden = 0,
                        AmountNonGolden = 0,
                        CardId = c.Id,
